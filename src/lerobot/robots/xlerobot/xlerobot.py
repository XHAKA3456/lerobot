...
class XLerobot(Robot):
    ...
    def __init__(self, config: XLerobotConfig):
        ...
        self._head_calibration_warning_logged = False

    def _get_calibrated_head_motors(self) -> list[str]:
        """Return head motors that currently have calibration data cached on bus1."""
        calibrated = [motor for motor in self.head_motors if motor in self.bus1.calibration]
        if (
            self.head_motors
            and len(calibrated) != len(self.head_motors)
            and not self._head_calibration_warning_logged
        ):
            missing = [motor for motor in self.head_motors if motor not in self.bus1.calibration]
            logger.warning(
                "Missing calibration for head motors %s. Head control/telemetry will be disabled "
                "until you run calibration.",
                missing,
            )
            self._head_calibration_warning_logged = True
        return calibrated
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        head_pos = {}
        calibrated_head = self._get_calibrated_head_motors()
        if calibrated_head:
            head_pos = self.bus1.sync_read("Present_Position", calibrated_head)
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        calibrated_head = self._get_calibrated_head_motors()
        if calibrated_head:
            allowed_head_keys = {f"{motor}.pos" for motor in calibrated_head}
            head_pos = {k: v for k, v in head_pos.items() if k in allowed_head_keys}
        else:
            head_pos = {}
        ...
        if self.config.max_relative_target is not None:
            ...
            present_pos_head = (
                self.bus1.sync_read("Present_Position", calibrated_head) if calibrated_head else {}
            )
